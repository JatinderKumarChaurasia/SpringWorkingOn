...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/").permitAll() // Allow Public
																// Access to
																// Home
				.antMatchers("/employees").hasRole("EMPLOYEE").antMatchers("/leaders/*").hasRole("MANAGER")
				.antMatchers("/systems/**").hasRole("ADMIN").and().formLogin().loginPage("/showMyLoginPage")
				.loginProcessingUrl("/authenticateTheUser").permitAll(true).and().logout().logoutSuccessUrl("/")// After
																												// Logout
																												// Redirect
																												// to
																												// Login
